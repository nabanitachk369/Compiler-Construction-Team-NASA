...
lex.yy.c: $(LEX_SRC) $(PARSER_HEADER)
	$(LEX) -o $@ $^
...
ast.o: $(AST_SRC) $(AST_DIR)/ast.h
	$(CC) $(CFLAGS) -c $< -o $@

symbol_table.o: $(SYM_SRC) $(SYM_DIR)/symbol_table.h
	$(CC) $(CFLAGS) -c $< -o $@

semantic.o: $(SEM_SRC) $(SEM_DIR)/semantic.h
	$(CC) $(CFLAGS) -c $< -o $@

tac.o: $(CODEGEN_SRC) $(CODEGEN_DIR)/tac.h
	$(CC) $(CFLAGS) -c $< -o $@
...
test-invalid: $(TARGET)
	...
	@for file in tests/invalid/*.md; do \
		echo "Testing $$file..."; \
		./$(TARGET) $$file || true; \
		echo ""; \
	done

.PHONY: all clean distclean test test-invalid
